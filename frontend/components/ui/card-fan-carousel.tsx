...
interface SocialCardsProps {
  cards: CardItem[];
  autoRotateInterval?: number; // Optional auto-rotate time in ms
}
...
export function SocialCards({ cards, autoRotateInterval = 3000 }: SocialCardsProps) {
  ...
  // Always enable pagination/cycling if there's more than 1 card, so it can auto-rotate.
  ...
  const getVisibleMap = useCallback((center: number) => {
    ...
    // If we have fewer than MAX_VISIBLE, we still map them to slots around the center
    const visibleCount = Math.min(totalCards, MAX_VISIBLE);
    const startSlot = Math.floor((MAX_VISIBLE - visibleCount) / 2);
    
    for (let i = 0; i < visibleCount; i++) {
      const cardIndex = (((center + i - Math.floor(visibleCount / 2)) % totalCards) + totalCards) % totalCards;
      map.set(cardIndex, startSlot + i);
    }
    return map;
  ...
  // Auto-rotate logic
  useEffect(() => {
    if (!needsPagination || autoRotateInterval <= 0) return;
    
    const intervalId = setInterval(() => {
      if (!isHovering.current && !isAnimating.current) {
        cycle("right");
      }
    }, autoRotateInterval);

    return () => clearInterval(intervalId);
  }, [needsPagination, autoRotateInterval, cycle]);
  ...
  useEffect(() => {
    ...
    // Fix: Use a failsafe timeout to reset isAnimating instead of relying on GSAP onComplete
    // which can be overwritten/killed by hover events, permanently stopping the auto-rotate.
    const maxDuration = isFirstMount ? 1200 : 800; // Increased duration for smoother glide
    ...
    cardElements.forEach((card, cardIndex) => {
      ...
      if (slot !== undefined) {
        ...
        if (isFirstMount) {
          ...
          gsap.to(card, { ...target, duration: 1.2, ease: "power2.out", delay: 0.2 + slot * 0.06 });
        } else if (!wasVisible) {
          const enterX = direction === "right" ? 40 : -40;
          gsap.set(card, { x: `${enterX}rem`, y: `${y * hMult}rem`, rotation: direction === "right" ? 30 : -30, scale: 0.5, opacity: 0 });
          gsap.to(card, { ...target, duration: 0.8, ease: "power1.inOut" });
        } else {
          // Make the transition much smoother and longer to feel like continuous rotation
          gsap.to(card, { ...target, duration: 0.8, ease: "power1.inOut" });
        }
      } else if (wasVisible) {
        const exitX = direction === "right" ? -40 : 40;
        gsap.to(card, { x: `${exitX}rem`, opacity: 0, scale: 0.5, rotation: direction === "right" ? -30 : 30, duration: 0.8, ease: "power1.inOut", zIndex: 0 });
    ...
    const updateHoverLayout = (hoveredSlot: number | null) => {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        gsap.to(el, {
          ...
          duration: 0.5, delay, ease: "power2.out", overwrite: "auto",
        });
    ...
    const onContainerEnter = () => { isHovering.current = true; };
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        {/* We fix the height so the absolutely positioned cards don't collapse the layout */}
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[550px] md:h-[600px]">
          {cards.map((card, index) => {
            return (
              <div key={card.id || index} className="fan-card absolute" style={{ width: '360px', height: '480px' }}>
                <div className="w-full h-full relative z-10">{card.content}</div>
              </div>
            );
          })}
        </div>
