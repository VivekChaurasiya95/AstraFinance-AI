      ...
      {selectedFiles.length > 0 && (
        <div className="mt-8 space-y-3">
          ...
          {selectedFiles.map((file, i) => (
            <div
              key={i}
              className="flex items-center justify-between bg-card border border-border rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow"
            >
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button
              ...
              className="px-6 py-2 text-sm font-medium text-white bg-primary rounded-xl hover:bg-primary/90 flex items-center gap-2 transition-colors"
            >
...
function DocumentDetailPanel({
  ...
  return (
    <div className="w-[380px] bg-card h-full border-l border-border flex flex-col shrink-0 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-border-subtle">
        <h3 className="text-sm font-bold text-foreground truncate pr-2 flex-1">
          {doc.name}
        </h3>
        <button
          onClick={onClose}
          className="p-1.5 text-muted-foreground hover:text-foreground rounded-lg hover:bg-surface transition-colors shrink-0"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-surface rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-foreground">{doc.pages}</p>
            <p className="text-[10px] text-muted-foreground font-medium">Pages</p>
          </div>
          <div className="bg-surface rounded-xl p-3 text-center">
          ...
          <div className="bg-surface rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-foreground">{fileExt}</p>
        ...
        {doc.extracted_metrics && doc.extracted_metrics.length > 0 && (
          <div className="mb-6">
            ...
            <button className="text-xs text-primary hover:text-primary-hover font-medium mt-2 flex items-center gap-1">
              View all metrics →
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-2 mt-4">
          <button className="flex items-center justify-center gap-1.5 text-xs font-medium text-primary bg-primary/10 border border-primary/50 px-3 py-2.5 rounded-xl hover:bg-primary/20 transition-colors">
    ...
    return (
      <div className="p-6 flex flex-col gap-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            className="flex items-center gap-4 bg-card border border-border rounded-xl p-4 animate-pulse"
          >
      ...
      <div className="flex-1 flex flex-col p-6 min-w-0">
        ...
        {layoutMode === "table" ? (
          <div className="flex-1 bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-border">
              {filteredDocs.length === 0 ? (
                ...
                filteredDocs.map((doc) => (
                  <div
                    key={doc.id}
                    className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-surface transition-colors cursor-pointer group"
            ...
            {/* Footer */}
            {filteredDocs.length > 0 && (
              <div className="flex items-center justify-between px-6 py-3 border-t border-border-subtle bg-surface/50">
                <span className="text-xs text-muted-foreground">
                  Showing 1–{filteredDocs.length} of {filteredDocs.length}{" "}
                  documents
                </span>
                <div className="flex items-center gap-1">
                  <button className="p-1.5 text-muted-foreground hover:text-foreground rounded-lg hover:bg-surface">
            ...
            {filteredDocs.length === 0 ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredDocs.map((doc) => (
                  <div
                    key={doc.id}
                    className="bg-card border border-border rounded-xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer group hover:border-primary/50"
                    onClick={() => setSelectedDoc(doc)}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-12 h-12 rounded-xl bg-destructive/10 border border-red-100 flex items-center justify-center shrink-0">
                      ...
                      <button
                        ...
                        className="p-1 text-muted-foreground hover:text-destructive rounded-lg hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-all"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                    <h4 className="text-sm font-semibold text-foreground truncate mb-1 group-hover:text-primary transition-colors">
                      {doc.name}
                    </h4>
